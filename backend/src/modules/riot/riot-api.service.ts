...
import { getRiotStats, saveRiotStats } from './riot-stats.repository.js';
...
async function getDataDragonVersion(): Promise<string> {
  try {
    ...
    Logger.error('Error fetching Data Dragon version:', error);
...
export async function fetchSummonerInfoFromApi(
  ...
  try {
    ...
    try {
      ...
      Logger.info('No rank data found');
    }
...
export async function getSummonerInfo(gameName: string, tag: string): Promise<RiotResponse> {
  ...
  Logger.info(`Riot stats absentes en base, fetch API (${gameName}#${tag})`);
